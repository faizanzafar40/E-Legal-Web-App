	...
	<div id="my_blog">
		...
		<div id="columns">
			<?php
				$stmt = $db->prepare("SELECT * FROM questions WHERE client_name = :me AND question_id IN (SELECT question_id FROM answers)");
				$stmt->execute(array(':me' => $me));
				$answered = $stmt->fetchAll();
				for($i = 0; ($i < 9) && ($i < count($answered)); $i++) {
					$row = $answered[$i];
					echo"
	...
	<div id="my_blog">
		...
		<div id="columns">
			<?php
				$stmt = $db->prepare("SELECT * FROM questions WHERE client_name = :me AND question_id NOT IN (SELECT question_id FROM answers)");
				$stmt->execute(array(':me' => $me));
				$unanswered = $stmt->fetchAll();
				for($i = 0; ($i < 9) && ($i < count($unanswered)); $i++) {
					$row = $unanswered[$i];
					echo"
					<figure id=\"" . $row['question_id'] . "\">
						<figcaption>" .  $row['title'] . "</figcaption>
