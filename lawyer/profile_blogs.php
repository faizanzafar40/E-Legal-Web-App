	...
	// Articles are listed for the demo author "murtaza" that ships with the
	// sample database, highest rated first.
	$name = "murtaza";
	$stmt = $db->prepare("SELECT * FROM articles WHERE username = :name ORDER BY rating DESC");
	...
	<div id="columns">
		<?php 
		for($i = 0; ($i < 9) && ($i < count($articles)); $i++) {
			$row = $articles[$i];
